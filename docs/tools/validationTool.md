...
**概要**：2つの任意値が等しいかどうかを判定します。オブジェクトや配列は深い比較で評価します。内部的には `lodash.isEqual` の比較ルールに従います。
...
| パラメータ名 | 型 | 必須 | 説明 |
|---|---|---|---|
| value1 | any | ✅ | 比較対象の1つ目の値 |
| value2 | any | ✅ | 比較対象の2つ目の値 |

**戻り値**

| 型 | 説明 |
|---|---|
| boolean | 2つの値が等しい場合は `true`、等しくない場合は `false` |

**比較ルール（主な例）**

- プリミティブ値: 値が同じなら `true`
- オブジェクト/配列: 深い比較（ネストを含む）
- `Date`: 同一時刻なら `true`
- `NaN`: 同士は `true`
- 異なる型（例: `'123'` と `123`）は `false`

**使用例**

```js
isEqualValue(100, 100); // true
isEqualValue({ a: 1, b: [2, 3] }, { a: 1, b: [2, 3] }); // true
isEqualValue([1, 2, 3], [3, 2, 1]); // false
isEqualValue(new Date('2026-01-01'), new Date('2026-01-01')); // true
isEqualValue('123', 123); // false
```
